...
function categorizeEvent(title: string) {
  for (const category of categories as CategoryType[]) {
    if (
      category.subCategories.some((sub: string) =>
        title.toLowerCase().includes(sub.toLowerCase()),
      )
    ) {
      return category;
    }
...
export function EventCard({ event, isFavorite, onFavorite }: EventCardProps) {
  ...
  return (
    <Card
      className={`mb-4 overflow-hidden border-l-4 hover:shadow-lg transition-all duration-200 ${color}`}
    >
